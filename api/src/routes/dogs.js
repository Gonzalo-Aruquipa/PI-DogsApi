...
router.get("/", async (req, res) => {
  ...
  try {
    if (name) {
      const filterDogs = dbApi.filter((dog) =>
...
router.get("/api", async (req, res) => {
  const response = await getData();
  const dogMain = response.map((dog) => {
    return {
      id: dog.id,
      name: dog.name,
      temperaments: dog.temperaments,
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const newDog = await Dog.create({
      ...
      image: image
        ? image
        : "https://img.freepik.com/vector-premium/huella-vector-pata-perro_71328-325.jpg?w=2000",
    });
